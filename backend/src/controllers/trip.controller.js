...
export const dispatchTrip = asyncHandler(async (req, res) => {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    // Vehicle/driver writes must land BEFORE the trip's include-read below —
    // within one transaction, statements are serialized on a single
    // connection in dispatch order, so firing the include-read first (e.g.
    // via Promise.all with array order [trip, vehicle, driver]) reads their
    // pre-update status even though this callback returns after all three
    // resolve. Sequential awaits, writes-then-read, avoids that.
...
export const completeTrip = asyncHandler(async (req, res) => {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    // Writes before the trip's include-read — see the comment in
    // dispatchTrip on why order matters within one transaction.
